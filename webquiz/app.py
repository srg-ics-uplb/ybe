...
import uuid
import sqlite3
from config import MAX_LOGIN_ATTEMPTS, YBE_FILE, QUIZ_TITLE
...
def login():
    if request.method == 'POST':
        ...
        if user:
            # Check exam code matches user's assigned code
            if quiz_code == user['quiz_code']:
                ...
                db.execute(
                    'UPDATE user_sessions SET session_id = ? WHERE email = ?',
                    (session_id, email)
                )
                db.commit()
                ...
                logger.warning(f"Invalid quiz code for {email}")
                return render_template('login.html',
